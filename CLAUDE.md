...
プロジェクトを指すときは、必ずその**開発場所（`pmo/` があるPC＝Main のPC）を明記する**こと。

- 例: 「air-conditioner-replace（開発場所: MHBook-Air の `~/dev/air-conditioner-replace/`）」
- どのPCにも `pmo/` がない場合は、存在するPC名を挙げた上で「Main 未設定」と明記する
- ユーザーは複数PCを使い分けているため、どのPCで Claude を立ち上げるべきかが常に分かる状態にする
...
**ここはPMO（Project Management Office）であり、実作業の場ではない。**

- やること: プロジェクトの状況把握・課題の整理・次にやることの検討
- やらないこと: 各プロジェクトのコードを書く・ファイルを編集する・コミットする

実際の作業は、各プロジェクトのディレクトリ（`~/dev/{project-name}/`）で Claude を別途立ち上げて行う。
ユーザーが「〇〇をやろう」と決めたら、そのプロジェクトのディレクトリで Claude を起動するよう促すこと。
